...
const getConfig = () => {
    ...
    const BATCH_RPC_PROVIDERS = [ANKR_URL, RPC_URL, PUBLIC_SOLANA_URL].filter(Boolean) as string[];
    const RPC_PROVIDERS = [HELIUS_URL, ALCHEMY_URL, ANKR_URL, RPC_URL, PUBLIC_SOLANA_URL].filter(Boolean) as string[];

    return { RPC_PROVIDERS, BATCH_RPC_PROVIDERS, isValidHeliusKey };
};
...
export const getCurrentRPC = () => {
    const { RPC_PROVIDERS, isValidHeliusKey } = getConfig();
...
export const getBatchRPC = () => {
    const { BATCH_RPC_PROVIDERS } = getConfig();
